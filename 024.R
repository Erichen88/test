...
data <- read.table("TRD_Index2.txt",header=TRUE) 
...
SHmod <- auto.arima(return,stationary=TRUE,
                    seasonal=FALSE,ic='aic')

summary(SHmod) 
confint(SHmod)

SHmod2=arima(return,order=c(2,0,2),include.mean=F)
summary(SHmod2)
confint(SHmod2)
tsdiag(SHmod2)

Box.test(return,type="Ljung-Box",fitdf=0,lag=12)
